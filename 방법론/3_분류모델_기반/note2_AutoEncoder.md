...
|장점|단점|
|:---|:---|
|1. 데이터 Label이 존재하지 않아도 사용 가능, but 정상적인 데이터를 추정할 수 있을 정도는 되어야 함 <br/> 2. 고차원 데이터 특징을 추출 가능 <br/> 3. 오토인코더 기반의 다양한 알고리즘 존재, 웹상에 많은 레퍼런스 존재 |1. 히든레이어 설정(하이퍼 파라미터) <br/> 2. Loss(MSE)에 대한 threshold 설정이 어려움|
