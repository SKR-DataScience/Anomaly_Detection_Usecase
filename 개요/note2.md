...
|훈련셋||훈련 데이터 내 정상, 이상치 샘플 모두 있음|-이상치 없음 <br/>- **학습 데이터가 이상치에 의해 오염되지 않았다고 가정** <br/> - 훈련셋 내 모든 샘플과 다른 새로운 샘플 탐지가 목적, 훈련셋 내 샘플은 oultㅑ
|작동방식||- 중앙(데이터 중심부 피팅)에서 벗어난 데이터를 이상치로 판단|- "Out of Distribution": 모르는 분포에서 발생한 문제 탐지 <br/> - 

1. Outlier Detection
- 이상치가 현업에서 문제를 일으키면 이상치(outlier)가 이상(abnormal)
- 학습 데이터를 통해 정상 범위 결정, 범위 밖 데이터를 이상으로 간주
- 훈련 데이터 내 정상, 이상치 샘플 모두 잇음
- 중앙(데이터 중심부 피팅)에서 벗어난 데이터를 이상치로 판단
2. Novelty Detetction
- 학습 데이터가 이상치에 의해 오염되지 않았다고 가정
- 새로운 패턴 데이터를 찾기(리스크와 관련 없을수도 있음)
- ex. 개 종류 분류 문제에 고양이 사진을 테스트하면 개 종류 중 가장 유사한 강아지를 결과로 반한, unknown 클래스 탐지 불가능 $\rightarrow$ "Out of Distribution"(모르는 분포에서 발생한 문제)
3. 
- 현업의 룰에 의해 이상 정의 후 탐지
