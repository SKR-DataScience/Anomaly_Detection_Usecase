...
class RCTree:
    ...
    def _mktree(self, X, S, N, I, parent=None, side='root', depth=0):
        ...
        # Otherwise...
            ...
            i = np.ndarray.item(np.flatnonzero(S1)) #np.asscalar(np.flatnonzero(S1))
        ...
        # Otherwise...
            ...
            # Create a leaf node from isolated point
            i = np.ndarray.item(np.flatnonzero(S2)) # np.asscalar(np.flatnonzero(S2))
            leaf = Leaf(i=i, d=depth, u=branch, x=X[i, :], n=N[i])
            # Link leaf node to parent
            branch.r = leaf
            # If duplicates exist...
    ...
    def _count_leaves(self, node):
        ...
        num_leaves = np.ndarray.item(num_leaves) #np.asscalar(num_leaves)
        return num_leaves
